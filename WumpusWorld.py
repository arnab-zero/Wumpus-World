...
class WumpusWorld():
    ...
    def run ( self, screen ):
        ...
        while self.__score >= -1000:
            ...
            refreshBoardGraphics(self.__board, self.__agentDir, show_board, screen)
            self.__board[self.__agentX][self.__agentY].agent = False

                        
            # Get the move
            self.__lastAction = self.__agent.decideAction (self.__board[self.__agentX][self.__agentY].stench,self.__board[self.__agentX][self.__agentY].breeze,
    ...
    def __addBoardFeatures ( self, file = None ):
        if file == None:
            
            
            # Generate wumpus
            wc = self.__generateRandomInt(self.__colDimension)
            wr = self.__generateRandomInt(self.__rowDimension)
            ...
            # Generate gold
            gc = self.__generateRandomInt(self.__colDimension)
            gr = self.__generateRandomInt(self.__rowDimension)
            ...
            for r in range (self.__rowDimension):
                for c in range (self.__colDimension):
                    if (c != 0 or r != 0) and self.__generateRandomInt(30) < 2 and (self.__board[c][r].wumpus == False) and (self.__board[c][r].gold == False):
                        self.__addPit ( c, r )
        ...
        self.__agent = AI(self.__numberOfGolds, self.__numberOfArrows)
